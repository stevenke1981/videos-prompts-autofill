...
export const BanksSidebar = React.memo(({ 
  ...
  const filteredBanks = useMemo(() => {
    ...
    return Object.fromEntries(
      Object.entries(banks).filter(([key, bank]) => {
        const label = getLocalized(bank.label, language).toLowerCase();
        const optionsMatch = (bank.options || []).some((opt) =>
          getLocalized(opt, language).toLowerCase().includes(query)
        );
        return key.toLowerCase().includes(query) || label.includes(query) || optionsMatch;
      })
    );
  }, [banks, bankSearch, language]);
      ...
      <div className="p-5 border-b border-white/30 bg-white sticky top-0 z-30 shadow-sm">
        ...
        <div className="relative mt-3">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-xs bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-300 transition-all"
          />
        </div>
      </div>
