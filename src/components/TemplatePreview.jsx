...
export const TemplatePreview = React.memo(
  ({
    ...
    const rawImageUrl = allImages[editImageIndex] || activeTemplate?.imageUrl;
    const [currentImageUrl, setCurrentImageUrl] = React.useState(rawImageUrl);

    React.useEffect(() => {
      let cancelled = false;
      const load = async () => {
        if (!rawImageUrl) {
          setCurrentImageUrl(rawImageUrl);
          return;
        }
        if (isIdbReference(rawImageUrl)) {
          const resolved = await resolveImageUrl(rawImageUrl);
          if (!cancelled) setCurrentImageUrl(resolved);
        } else {
          setCurrentImageUrl(rawImageUrl);
        }
      };
